...
Last updated: 2026-09-21 · Current version: 1.4.2 · Tests: 479 passing (+1 skipped on Windows by design) · Lint: ruff (E4,E7,E9,F,I,B,UP,SIM,RUF,BLE001,S110,S112) + mypy clean · No `src/` file >25 KB except 4 tracked borderline files · Phase 0 exit criteria met · Phase 2.2 migration tooling shipped · Phase 2.3 store-first cutover shipped (flat fact-writes removed) · Phase 3 capture reliability (filter, roll-up, multi-client hooks) shipped · v1.1 project directives (sync/context routing, managed markers, `sync-agents --check`) shipped · v1.1.2 instruction-template graceful degradation shipped · v1.2.0 conflict-free team merges shipped (generated-view union merge, block-level store merge, `resolve-conflicts`) · v1.3.0 maps-first retrieval + Cursor hooks + coding-memory bench shipped · v1.4.1 field write/read hygiene from dogfood stores shipped · v1.4.2 field pack precision (contradictions, stale ranking, dream churn) shipped
...
**Implemented 2026-07-17** — narrowed to the directive tier (the roadmap's own
"or narrowed to the directive tier only" option), not fully deleted, because the
steering sections (`framework-rules`, `ubiquitous-language`) legitimately need a
hand-write path and the `write_local_memory` CRUD engine is still the one that
serves them. The enforcement lives at the MCP tool boundary: the tool is the only
production caller of `write_local_memory` (maps are regenerated by `maps.py`
directly), so guarding the tool closes the agent-facing fact-write path without
touching map regeneration or migration. Version bump + tag deferred to the
release step.

- [x] `write_local_memory_tool` narrowed to the directive tier only:
      `flat_write_rejection` (in `storage/sections.py`) rejects a write to any
      generated map section (`architecture`, `decisions`, `debt`, `schemas`,
      `index`) or arbitrary fact section, pointing the caller at
      `write_memory_store_tool`; only steering sections (the two canonical names,
      or content/file declaring `role: steering`) remain writable.
      `read_section_tool` stays — maps are still readable files.
- [x] `ai-memory doctor` flags legacy hand-written root sections
      (`_check_legacy_flat_sections`): a map-category file without
      `generated: true` frontmatter warns and points to `ai-memory migrate`.
      Clean on a fresh `init` (scaffolded maps are `generated: true`) and on this
      repo's migrated store.
- [x] `.ai-memory/` root contains only generated files (maps, `index.md`,
      `consolidated_memory.md` cache), directives, and the store/journal/snapshot
      dirs — the closed write path plus the doctor flag together enforce it.
...
- [x] **Optional local field diary + recording (no upload)** — v1.4.0: human
      `ai-memory diary approve`, local JSONL `PackStats` / tool / session events.
      `diary pack` / maintainer analyze remain follow-up. Nothing is transmitted.
