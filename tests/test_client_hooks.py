...
Covers the generalized `install_hooks()` registry/dispatcher and the
claude-code adapter that writes SessionStart/Stop/PreCompact into
`.claude/settings.json`, plus the CLI wiring (`install --with-hooks` and
`session-start --hook-format claude-code`).
"""
