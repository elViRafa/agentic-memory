...
if FastMCP is not None:
    ...
    async def apply_dream_results_tool(
        ...
        Args:
            cwd:               Absolute path to the project root.
            candidate_store:   ``candidate_store`` value returned by
                               ``prepare_dream_payload_tool``.
            llm_response:      Raw JSON consolidation response from the client's LLM.
            mode:              Same as ``dream_tool``.
            apply:             Persist changes to live memory when ``True``.
            llm_rewrite:       Generate rewrite tasks for low-quality sections.
            max_rewrite_tasks: Maximum number of sections to rewrite per run.
            with_eval:         Run a quality evaluation after applying changes
                               (requires ``apply=True``).
            save_report:       Persist the evaluation report to disk.
        ...
        """
        safe = _safe_cwd(cwd)
        result = await apply_dream_results(
            safe,
