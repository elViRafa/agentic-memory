...
def _steering_routing_flags(path: Path) -> tuple[bool, bool]:
    """Effective ``(sync, context)`` routing flags for a steering section file.

    Both keys are optional booleans in the file's frontmatter (v1.1):

    - ``sync``: include the directive in the per-tool files generated by
      ``sync-agents``. Defaults to True.
    - ``context``: inject the directive as Tier 0 into ``read_combined_context``.
      Defaults to True only for the canonical ``STEERING_SECTIONS`` names
      (preserves v1.0.0 behavior); False for every other steering section,
      because file-reading agents already receive the synced copy and
      double-injection wastes context.

    Non-boolean values fall back to the defaults here (``doctor`` reports them
    as errors); routing must stay predictable even on a malformed file.
    """
    try:
        metadata, _body = parse_frontmatter(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, FrontmatterError):
        metadata = {}
    sync_value = metadata.get("sync")
    sync = sync_value if isinstance(sync_value, bool) else True
    context_value = metadata.get("context")
    context = context_value if isinstance(context_value, bool) else path.stem in STEERING_SECTIONS
    return sync, context


def _steering_sync_enabled(path: Path) -> bool:
    """Whether a steering section participates in `sync-agents` output."""
    return _steering_routing_flags(path)[0]


def _steering_context_enabled(path: Path) -> bool:
    """Whether a steering section is injected as Tier 0 context."""
    return _steering_routing_flags(path)[1]
